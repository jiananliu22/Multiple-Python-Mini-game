...
pygame.display.set_caption('Trace Car')
...
def game_intro():
    ...
    while intro:
        ...
        intro_text = intro_font.render('Trace Car', True, black)
        gameDisplay.blit(intro_text, [125, 200])

        illu_font = pygame.font.SysFont("comicsansms",34)
        illu_text = illu_font.render("let's trace youth!!", True, black)
        illu_text_2 = illu_font.render("sapce can pause the game.", True, black)
        gameDisplay.blit(illu_text, [160, 380])
        gameDisplay.blit(illu_text_2, [145, 430])
...
def game_loop():
    ...
    while not gameExit:
        ...
        for i in range (len(thing_x)): 
            #thing_startx = random.randrange(0, display_width)
            things(thing_x[i], thing_starty, thing_width, thing_height, thing_color)
            thing_x[i] += thingx_speed_group[i]
        ...
        if thing_starty >= display_height:
            ...
            if score < 60:
            	...
            	thingx_speed_group = []
            	for i in range (3):
            		thingx_speed = random.randrange(-5, 5)
            		thingx_speed_group.append(thingx_speed)
            ########
